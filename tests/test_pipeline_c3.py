...
C3_CAVEATS = slice_caveats("c3")
C3_CITATIONS = slice_citations("c3")
C3_GEOGRAPHIES = slice_geographies("c3")
    ...
    *,
    ...
    for binding in C3_SERIES:
        ...
        if binding.series_id not in NAMED_HOLE_SERIES_IDS:
            ...
            table.write_text(
                _csv_for(binding.series_id), encoding="utf-8", newline="\n"
            )
...
def test_blank_budget_cell_is_unknown_not_zero() -> None:
    ...
    blank = [
        ...
        if item.observation_id.endswith("-cgst")
        and item.reference_period == "budget-2025-2026"
    ]
    assert len(blank) == 1
...
def test_be_re_actuals_are_reference_periods_not_geography() -> None:
    ...
    corporation = [
        item
        for item in observations
        if item.observation_id.endswith("-corporation-tax")
    ]
...
def test_empty_line_label_is_skipped() -> None:
    rows = [
        ...
        {
            "line_label": "Fiscal Deficit",
            "actuals_2024_2025": "(4.8%)",
            "budget_2025_2026": "5",
            "revised_2025_2026": "6",
            "budget_2026_2027": "7",
        },
    ...
    actuals = [
        item for item in observations if item.reference_period == "actuals-2024-2025"
    ]
...
def test_h1_lineage_ok_no_does_not_write_vintage(tmp_path: Path) -> None:
    ...
    with pytest.raises(PipelineError, match="lineage_ok"):
        materialise_vintage(data_root, logs_root, slice_id="c3", created_at=CREATED_AT)
...
def test_first_vintage_lists_eleven_names_card_8_hole_and_leaves_pointers(
    ...
    manifest, report = materialise_vintage(
        data_root, logs_root, slice_id="c3", created_at=CREATED_AT
    )
    ...
    hole = next(
        ...
        if entry.series_id == SERIES_BUDGET_2026_27_FRBM_STATEMENTS
    )
    assert hole.lineage_ok is YesNo.no
    parquet = (
        series_dir(
            data_root, manifest.vintage_id, SERIES_BUDGET_2026_27_FRBM_STATEMENTS
        )
    ...
    assert (
        loaded[0].citation_id
        == C3_CITATIONS[SERIES_BUDGET_2026_27_FRBM_STATEMENTS].citation_id
    )
...
def test_second_vintage_hard_links_unchanged_series(tmp_path: Path) -> None:
    ...
    second, report_b = materialise_vintage(
        data_root, logs_root, slice_id="c3", created_at=LATER
    )
...
def test_c1_vintage_is_not_previous_for_c3_reuse(tmp_path: Path) -> None:
    ...
    materialise_vintage(data_root, logs_root, slice_id="c1", created_at=LATER)
    second, _report_b = materialise_vintage(
        data_root, logs_root, slice_id="c3", created_at=AFTER_C1
    )
    assert first.vintage_id != second.vintage_id
