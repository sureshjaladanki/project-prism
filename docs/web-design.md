...
Tone is aligned with how USAFacts answers a question — modern, welcoming, civic publisher, generous ([design-philosophy.md](design-philosophy.md)). Look follows house rules H1–H7 there and in [design-system.md](design-system.md): own lockup and tokens; we do not use another publisher's wordmark, magenta, or Aeonik. This file is the *machine* — paths, catalog, serve, SEO. Do not copy `/answers/…/country/united-states/`, a search box, newsletter, or media desk. Search is **refused this wave**; Front-end Architect may reopen when eight or more slices are published.

The philosophy's first two guidelines are IA, not just look: **the answer is the product** (question → thesis → movement → what-it-is → figures → evidence → method — see [Scan path as IA](#scan-path-as-ia-not-as-type)), and **invitation, then depth**. Tokens, well composition, and serif/sans stay in [design-system.md](design-system.md). Citizen questions and thesis copy stay in [editorial-guidelines.md](editorial-guidelines.md). This file does not invent a second tone, run a presentation pass, or add a geography route this wave.

Content Editor writes the citizen question and the thesis / movement / what-it-is. This file maps those words onto routes, `<title>`, meta, and chrome. It does not invent a second question.
...
Do not generate state folders this wave. Charter names which slices before this file reopens the path. Front-end row S3 and Charter ruling 7 ([citizen-page-feedback.md](archive/citizen-page-feedback.md)): same reservation, stated once so the next Charter slice has a contract to build against.
...
**Header (IA signed).** Text lockup `Prism` always links to the desk home `/`, never to C1. Sleeve links list the five sleeves (a sleeve with no catalog slice still goes to its hub — quiet empty with one citizen invitation line, no invented figures). Current sleeve: `aria-current="page"` on that hub; on a slice, `aria-current="true"` on its sleeve link. No search (refused this wave; reopen at eight or more published slices), subscribe, social, or “Ask the data”. No geography picker until geography routes exist. Look of this nav is UI/UX after this sign — do not invent a second IA in the presentation pass.
...
The slice's region order — sleeve → citizen question → byline → answer well (thesis, movement, what-it-is, stats, cite) → first chart → hottest-rail → method — is named here so UI/UX Developer does not invent a second page type or a second route for "the answer" versus "the depth". It stays one URL (`/{sleeve}/{slice}`); the order is what makes it "invitation, then depth" on a single page. Type, well composition, and elevation for that order are [design-system.md](design-system.md) "First screen (slice)". This file only guarantees the order is the page, not a `/how-it-is-measured` split-off.
...
**Sleeve hub.** Hub H1, then each catalog slice in that sleeve as its citizen question. The optional one-line bound fact is `fact_lede_one_line`. Each `.sleeve-index` card carries `data-vintage-id` from that slice's catalog entry. Canonical of every number remains the slice URL. **Quiet-empty hub (F-empty-sleeve):** when the sleeve has no catalog slice on this desk, the hub is still 200 — H1 plus **one citizen invitation line** (Content Editor copy in a later F-row; until then a signed placeholder is allowed), links back to `/` and sibling sleeves, **no invented figures**, no catalog dump. Geography routes stay reserved and unbuilt.
...
Do not implement geography routes, sitemap, `robots.txt` in the tree, or archive paths until a second slice is ready to publish. Do not add a home `.hottest-rail`. Search stays refused until eight or more published slices.
