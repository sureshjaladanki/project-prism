...
This file governs copy, scan path, voice, and chart *meaning*. [presentation-charter.md](archive/presentation-charter.md) applies it to C1; it does not replace it. [design-system.md](design-system.md) implements the look (type, colour, layout, chart chrome) — UI/UX Developer owns and implements that file.
...
- **Slots, not typed figures.** Copy binds to observations at render. If the vintage has no observation, the page reads as unknown or not published — never as a remembered number. Do not invent figures. Bound figures use Indian digit grouping (`12,34,567.89`), applied at bind so the lede, the stat cell, and the `meta description` are the same string.
...
4. **Proof** — On the rendered page, read as a citizen first. Fail if the H1 is not answered; if a section is a codebook recap; if annex tokens, vintage ids, or base-year notation appear in the byline, lede, evidence, source-lines, or chart titles the reader sees (cards may still print the producer series). Then chrome as a scan, not art direction: scan path; house `.fact-lede`; source-line in view; full cards collapsed once with the numbers; no codebook before the first figure; hottest rail is featured slices; analysis byline beside the agency cite. Proof closes by naming every form-factor break it saw — a figure that outgrows its cell, a chart whose labels are unreadable at the width you are reading, mixed plot widths, leftover white in a `--card` well, bars that fatten with few categories, Vega titles instead of HTML H2/byline — as a list handed to UI/UX Developer. Naming a break is not fixing it: Content Editor does not restyle a chart, change a token, or reword a figure to make it fit.

Then UI/UX Developer runs the **presentation pass** against [design-system.md](design-system.md) on the rendered preview page. It is a required gate on every template that reaches Proof, not only when chrome or charts changed — a new template breaks form factor at a new data shape while the house is untouched. It is recorded as skipped only when the same template renders the same page at the same vintage. The pass answers Proof's form-factor list and the checklist in that file; it does not reopen sections, wording, or voice. Then Trust Auditor (cites, dates, holes, spin). The presentation pass is not a fifth Content Editor writing pass. Trust does not become a style editor; Content Editor does not print codebook tokens to pre-empt Trust.
